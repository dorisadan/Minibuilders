...
int DynamixelClass::moveSpeedSync(unsigned char ID1, unsigned char ID2, int Position, int Speed)
{
    char Position_H,Position_L,Speed_H,Speed_L;
    ...
    #define SYNCWRITE_TOTAL_DMIXELS  2
 
    Checksum = (~(BROADCAST_ID + (AX_GOAL_SP_LENGTH * SYNCWRITE_TOTAL_DMIXELS) + AX_SYNC_WRITE + AX_GOAL_POSITION_L + AX_POS_LENGTH + ID1 + ID2 + (SYNCWRITE_TOTAL_DMIXELS * (Position_L + Position_H + Speed_L + Speed_H))))&0xFF;
    ...
    sendData(AX_GOAL_SP_LENGTH * SYNCWRITE_TOTAL_DMIXELS); // check

    sendData(AX_SYNC_WRITE); // check

    sendData(AX_GOAL_POSITION_L);

    sendData(AX_POS_LENGTH); // check


	/* START Dynamixel Motor 1 */
    ...
    /* END Dynamixel Motor 1 */


    /* START Dynamixel Motor 2 */
...
int DynamixelClass::turnSync(unsigned char ID1, unsigned char ID2, bool SIDE, int Speed)
{
    char Position_H,Position_L,Speed_H,Speed_L;
    ...
    Checksum = (~(BROADCAST_ID + (AX_SPEED_LENGTH * SYNCWRITE_TOTAL_DMIXELS) + AX_SYNC_WRITE + AX_GOAL_SPEED_L + 2 + ID1 + ID2 + (SYNCWRITE_TOTAL_DMIXELS * (Speed_L + Speed_H))))&0xFF;
    ...
    sendData(AX_SPEED_LENGTH * SYNCWRITE_TOTAL_DMIXELS); // check AX_SPEED_LENGTH = 5 right? // (4+1)*2+4 abans
